...
const ExtratoConta = () => {
  ...
  const gerarExtrato = async () => {
    ...
    if (extrato !== undefined) setExtrato(extrato);
  };
  ...
  function formatarDinDin(valor) {
    const dindinFormatado = parseFloat(valor).toLocaleString("pt-BR", {
      style: "currency",
      currency: "BRL",
    });
  ...
  useEffect(() => {
    const fetchSaldo = async () => {
      ...
      const saldoFormatado = parseFloat(saldo).toLocaleString("pt-BR", {
        style: "currency",
        currency: "BRL",
      });
    ...
    <div className="container-solicitar">
      <>
        <Modal show={modalFiltrarIsOpen} centered size="sm">
          <Modal.Header>
            <Modal.Title style={{ color: "#DB4648" }}>
              Filtrar Período
            </Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <div className="container-data">
              <label className="mt-2 mr-2" style={{ color: "#3f3d56" }}>
                Data inicial{" "}
              </label>
              <div>
                <input
              ...
              <label className="mt-4 mr-3" style={{ color: "#3f3d56" }}>
                Data final{" "}
              </label>
              <div>
                <input
      ...
      <div className="title-solicitar">
        <h3 className="titulo-h5">
          {" "}
          <img alt="" src={pageExtrato}></img> Extrato Bancário
        </h3>
      </div>
            ...
            <tbody>
              {extrato.map((record, index) => (
                <tr key={index}>
                  ...
                  <td>
                    {record.CodigoContaOrigem === user.codigo ? (
                      <span style={{ color: "red" }}>
                        -{formatarDinDin(record.Valor)}
                      </span>
                    ) : (
