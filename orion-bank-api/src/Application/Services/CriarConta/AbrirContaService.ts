...
export class AbrirContaService implements IAbrirContaService {
  ...
  async ObterSolicitacoesAberturaDeConta(
    take: number,
    skip: number
  ): Promise<Array<SolicitacaoAberturaContaDto>> {
    ...
    const abrirContaRepository = new AbrirContaRepository();
    const registrosSolicitacao =
      await abrirContaRepository.ObterSolicitacoesAberturaDeConta(take, skip);
  ...
  async BuscarContaPorDocumentoFederal(
    ...
    const abrirContaRepository = new AbrirContaRepository();
    const conta = await abrirContaRepository.BuscarContaPorDocumentoFederal(
      documentoFederal.replace(/[^0-9]/g, "")
    );
  ...
  async EfetuarAberturaDeConta(
    contaDto: ContaDto,
    codigoSolicitacao: string
  ): Promise<void> {
    let th = this;

    contaDto.DocumentoFederal = contaDto.DocumentoFederal.replace(
      /[^0-9]/g,
      ""
    );
    ...
    if (
      codigoSolicitacao === undefined ||
      codigoSolicitacao === null ||
      codigoSolicitacao.trim() === ""
    ) {
  ...
  async SolicitacaoAberturaDeConta(contaDto: ContaDto): Promise<void> {
    ...
    await abrirContaRepository.SolicitacaoAberturaDeConta(
  ...
  private async ValidarCriacaoDeConta(contaDto: ContaDto): Promise<void> {
    ...
    const abrirContaRepository = new AbrirContaRepository();
    if (
      (await abrirContaRepository.BuscarDocumentoFederalExistente(
    ...
    if (
      contaDto.NomeCompleto === null ||
      contaDto.NomeCompleto.trim() === "" ||
      contaDto.NomeCompleto.length > 200 ||
      contaDto.NomeCompleto.length < 5
    ) {
      throw new Error(
        "Nome é obrigatório. Nome tem que estar entre 5 e 200 caracteres."
      );
    }
    ...
    if (
      ...
      throw new Error(
        "Email é obrigatório. Email tem que conter no máximo 100 caracteres."
      );
    }

    if (
      (await abrirContaRepository.BuscarContaPorEmail(contaDto.Email)) != true
    ) {
      throw new Error("Email já existente.");
    }

    if (
      contaDto.TelefoneCelular === null ||
      contaDto.TelefoneCelular.trim() === "" ||
    ...
    if (
      (await abrirContaRepository.BuscarContaPorTelefone(
        contaDto.TelefoneCelular
      )) != true
    ) {
    ...
    if (
      contaDto.Logradouro === null ||
      contaDto.Email.trim() === "" ||
      contaDto.Logradouro.length > 60
    ) {
      throw new Error(
        "Logradouro é obrigatório. Logradouro tem que conter no máximo 60 caracteres."
      );
    }

    if (
      contaDto.NumeroResidencial === null ||
      contaDto.NumeroResidencial.toString().trim() === ""
    ) {
